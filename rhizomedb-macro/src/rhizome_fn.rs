...
pub(crate) fn expand(input: RhizomeFunctionDecl) -> TokenStream {
    ...
    if let Some(agg_expr) = is_aggregate {
        ...
        tokens = quote! {
            ...
            impl #impl_generics ::rhizomedb::aggregation::AggregateGroupBy<#input_type #return_type> for HelperType #ty_generics
            #where_clause
            {
        ...
        tokens = quote! {
            ...
            impl #impl_generics ::rhizomedb::predicate::PredicateWhere<#input_type> for HelperType #ty_generics
            #where_clause
            {
    ...
    let arg_vars = args.iter().map(
        |StrictFnArg {
             ...
             colon_token,
             ty,
         }| { quote!(#name #colon_token ::rhizomedb::var::TypedVar<#ty>) },
    );
